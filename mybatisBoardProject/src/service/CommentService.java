...
public class CommentService {
	...
	public int getBoardComment(int bno, String userid) {
		int result = -1;
		try(SqlSession session = SqlSessionManager.getSqlSession()){
			...
			while (true) {
				...
				boardComments =  commentDao.selectByBno(bno, pager.getEndRowNo(),pager.getStartRowNo());
				...
				switch(cmd) {
					case "1":
						...
						break;
					case "2":
						insertComment(bno, userid);	// 댓글쓰기
						break;
					case "3":
						deleteComment(userid);			// 댓글 지우기
						break;
					case "4":
