...
public class BoardExample_Ch {
	public static void main(String[] args) {
		...
		while(run) {
			...
			System.out.println("-------------------------------------------------");
			//목록 출력
			for(int i=document.length-1; i>-1;i--) { // 내림차순 정렬을 위해 document를 역순으로 탐색하여 출력
			...
			//넘버 선택
			System.out.print("선택 : ");
			select_Num_Check = scan.nextLine();
			// 입력된 번호가 정수타입인지 판별함
			int check = 0;
			for(int i = 0; i<select_Num_Check.length();i++) { //문자열 하나씩 읽어가며 정수인지 판별
				if(Character.isDigit(select_Num_Check.charAt(i))) {
					check += 1; // check 변수 1 증가
				}
			}
			if(check == select_Num_Check.length()) { //
				select_Num = Integer.parseInt(select_Num_Check);
				...
				System.out.println("정수 번호를 알맞게 입력해주세요.");
			}
